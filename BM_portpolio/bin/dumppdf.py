#!/Users/leenanju/portpolio/BM_portpolio/bin/python3.11
...
def dumpxml(out: TextIO, obj: object, codec: Optional[str] = None) -> None:
    ...
    if isinstance(obj, PDFStream):
        if codec == "raw":
            # Convert bytes to string for text I/O
            raw_data = obj.get_rawdata()
            out.write(raw_data.decode('latin-1', errors='replace'))
        elif codec == "binary":
            # Convert bytes to string for text I/O
            binary_data = obj.get_data()
            out.write(binary_data.decode('latin-1', errors='replace'))
        else:
            out.write("<stream>\n<props>\n")
